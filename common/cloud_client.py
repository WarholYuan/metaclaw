...
class CloudClient(LinkAIClient):
    ...
    def _remove_weixin_credentials():
        ...
        cred_path = os.path.expanduser(
            conf().get("weixin_credentials_path", "~/.weixin_metaclaw_credentials.json")
        )
        try:
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        # Intercept CLI/slash commands before the agent runs
        try:
            from plugins import PluginManager
            mgr = PluginManager()
            instance = mgr.instances.get("METACLAW_CLI")
            ...
            logger.warning(f"[CloudClient] metaclaw_cli intercept failed: {e}")
...
SEND_WEB_SUBDIR = "metaclaw-send"


def copy_send_file(src_path: str, workspace_root: str) -> str:
    """Copy *src_path* into ``websites/metaclaw-send/`` and return its URL.
    ...
    send_dir = os.path.join(ws, "websites", SEND_WEB_SUBDIR)
    try:
