...
def test_bash_tool_gets_sandbox_when_enabled():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('config.conf') as mock_conf, \
            ...
            assert mock_bash.config.get("sandbox_workspace") == sandbox, "bash config should contain sandbox_workspace"
...
def test_bash_tool_ignores_sandbox_when_disabled():
    from bridge.agent_initializer import AgentInitializer
    from agent.tools.tool_manager import ToolManager
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('config.conf') as mock_conf, \
            ...
            assert mock_bash.cwd == workspace, f"Expected bash cwd={workspace}, got {mock_bash.cwd}"
            assert not mock_bash.config or "sandbox_workspace" not in mock_bash.config, "bash config should NOT contain sandbox_workspace when disabled"
            assert not os.path.exists(sandbox), "Sandbox directory should NOT be created when disabled"
