---
title: Architecture
description: MetaClaw 2.0 system architecture and core design
---

MetaClaw 2.0 has evolved from a simple chatbot into a super intelligent assistant with Agent architecture, featuring autonomous thinking, task planning, long-term memory, and skill extensibility.

## System Architecture

MetaClaw's architecture consists of the following core modules:

<img src="https://cdn.link-ai.tech/doc/metaclaw-agent-arch-en.jpg.jpg" alt="MetaClaw Architecture" />
...
| **CLI** | Command-line system providing terminal commands (`metaclaw`) and chat commands (`/`) for process management, skill installation, configuration, knowledge base management, and more |

## Agent Mode Workflow

When Agent mode is enabled, MetaClaw runs as an autonomous agent with the following workflow:
...
The Agent workspace is located at `~/metaclaw` by default and stores system prompts, memory files, and skill files:
...
Secret keys are stored separately in `~/.metaclaw` directory for security:
...
| `agent_workspace` | Workspace path | `~/metaclaw` |
