---
title: One-click Install
description: One-click install and manage MetaClaw with scripts
---

The project provides scripts for one-click install, configuration, startup, and management. Script-based deployment is recommended for quick setup.

Supports Linux, macOS, and Windows. Requires Python 3.7-3.12 (3.9 recommended).

## Install Command

<Tabs>
  <Tab title="Linux / macOS">
    ```bash
    bash <(curl -fsSL https://cdn.link-ai.tech/code/metaclaw/run.sh)
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
    irm https://cdn.link-ai.tech/code/metaclaw/run.ps1 | iex
    ```
  </Tab>
</Tabs>

The script automatically performs these steps:

1. Check Python environment (requires Python 3.7+)
2. Install required tools (git, curl, etc.)
3. Clone project to `~/MetaClaw`
4. Install Python dependencies and Cow CLI
5. Guided configuration for AI model and channel
6. Start service

By default, the Web console starts after installation. Access `http://localhost:9899` to begin chatting.

## Management Commands

After installation, use the `metaclaw` command to manage the service:

| Command | Description |
| --- | --- |
| `metaclaw start` | Start service |
| `metaclaw stop` | Stop service |
| `metaclaw restart` | Restart service |
| `metaclaw status` | Check run status |
| `metaclaw logs` | View real-time logs |
| `metaclaw update` | Update code and restart |
| `metaclaw install-browser` | Install browser tool dependencies |

See the [Commands documentation](/en/cli/index) for more details.

<Note>
  If the `metaclaw` command is not available, you can use `./run.sh <command>` (Linux/macOS) or `.\scripts\run.ps1 <command>` (Windows) as a fallback. Both are functionally equivalent.
</Note>
