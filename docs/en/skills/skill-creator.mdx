...
| `metadata.metaclaw.requires.bins` | System CLI tools that must be installed |
| `metadata.metaclaw.requires.env` | Required environment variables (all must be present) |
| `metadata.metaclaw.requires.anyEnv` | Multiple API keys — at least one must be set |
| `metadata.metaclaw.requires.anyBins` | Multiple tools — at least one must be installed |
| `metadata.metaclaw.always` | Set to `true` to always load, skipping dependency checks |
| `metadata.metaclaw.emoji` | Display emoji (optional) |
| `metadata.metaclaw.os` | OS restriction, e.g. `["darwin", "linux"]` |
