...
在对话里发送 API Key，Agent 会通过 `env_config` 工具自动保存到 `~/metaclaw/.env`，**不需要重启**就能生效。例如：
...
终端里对应的命令是 `metaclaw skill disable image-generation` / `metaclaw skill enable image-generation`。
