"""install-browser - Install Playwright + Chromium for the browser tool."""
...
def run_install_browser(
    ...
    Reused by ``metaclaw install-browser`` CLI and chat ``/install-browser``.
    ...
    stream("")
    stream(f"Browser tool ready! Restart {APP_NAME} to enable it.", "green")
    _phase(on_phase, f"🎉 全部步骤结束。请重启 {APP_NAME} 后使用 browser 工具。")
    return 0
