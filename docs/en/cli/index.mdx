...
description: MetaClaw command system — Terminal CLI and chat commands
---

MetaClaw provides two ways to interact via commands:

- **Terminal CLI** — Run `metaclaw <command>` in your system terminal for service management, skill management, and other operations
- **Chat Commands** — Type `/<command>` or `metaclaw <command>` in any conversation to check status, manage skills, adjust configuration, etc.

## Cow CLI

After deploying with the one-click install script, the `metaclaw` command is automatically available. For manual installations, run:

```bash
pip install -e .
```

Then use the `metaclaw` command from anywhere:
...
Usage: metaclaw <command>

Service:
  start     Start the MetaClaw service
  stop      Stop the MetaClaw service
  restart   Restart the MetaClaw service
...
| Command | Terminal (`metaclaw`) | Chat (`/`) |
| --- | :---: | :---: |
| help | ✓ | ✓ |
| version | ✓ | ✓ |
| status | ✓ | ✓ |
