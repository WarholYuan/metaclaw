...
チャットで API Key を送信すると、Agent が `env_config` ツールで `~/metaclaw/.env` に保存します。**再起動不要**でただちに反映されます。例：
...
ターミナルでは `metaclaw skill disable image-generation` / `metaclaw skill enable image-generation`。
