...
Send an API key in the chat and the Agent will save it to `~/metaclaw/.env` using the `env_config` tool — **no restart needed**. For example:
...
In the terminal: `metaclaw skill disable image-generation` / `metaclaw skill enable image-generation`.
